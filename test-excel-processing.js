const { ExcelProcessor } = require('./backend/etl/ExcelProcessor');
...
async function testExcelProcessing() {
  ...
  if (summary) {
    ...
    console.log(`   - Total cost: ${summary.totalCost.toLocaleString('en-US', { style: 'currency', currency: 'SAR' })}`);
    console.log(`   - Average cost per row: ${summary.avgCostPerRow.toLocaleString('en-US', { style: 'currency', currency: 'SAR' })}`);
